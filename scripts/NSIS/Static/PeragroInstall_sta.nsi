...
Section "Peragro Game Client" SecClient
  ...
  ;Create uninstaller
  WriteUninstaller "$INSTDIR\PT\Uninstall.exe"
...
Section "Start Menu Shortcuts" SecStartMenu
  ...
  !insertmacro MUI_STARTMENU_WRITE_BEGIN Application
    ...
    CreateShortCut "$SMPROGRAMS\$STARTMENU_FOLDER\Uninstall.lnk" "$INSTDIR\PT\Uninstall.exe"
    CreateShortcut "$SMPROGRAMS\$STARTMENU_FOLDER\Peragro Tempus.lnk" "$INSTDIR\PT\client_static.exe" -fs -mode=800x600
    CreateShortcut "$SMPROGRAMS\$STARTMENU_FOLDER\Peragro Tempus (windowed).lnk" "$INSTDIR\PT\client_static.exe" -mode=800x600
    CreateShortcut "$SMPROGRAMS\$STARTMENU_FOLDER\Peragro Tempus (safe mode).lnk" "$INSTDIR\PT\client_static.exe" -verbose=-scf -console 
