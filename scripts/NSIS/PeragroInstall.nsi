...
Section "Start Menu Shortcuts" SecStartMenu
  ...
  !insertmacro MUI_STARTMENU_WRITE_BEGIN Application
    ...
    CreateShortcut "$SMPROGRAMS\$STARTMENU_FOLDER\Peragro Tempus.lnk" "$INSTDIR\client_static.exe" -fs -mode=800x600
    CreateShortcut "$SMPROGRAMS\$STARTMENU_FOLDER\Peragro Tempus (windowed).lnk" "$INSTDIR\client_static.exe" -mode=800x600
    CreateShortcut "$SMPROGRAMS\$STARTMENU_FOLDER\Peragro Tempus (safe mode).lnk" "$INSTDIR\client_static.exe" -verbose=-scf -console 
...
Section "Uninstall"
  ...
  Delete  "$INSTDIR\client_static.exe"
  Delete  "$INSTDIR\config.cs"
  Delete  "$INSTDIR\vfs.cfg"
  ...
  Delete  "$INSTDIR\cally.zip"
  Delete  "$INSTDIR\shader.zip"
