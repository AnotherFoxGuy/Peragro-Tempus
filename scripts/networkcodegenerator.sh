...
#while [ "$(echo "$data" | sed -n -e "/<\!--/{s/^.*<\!--.*-->.*\$/[Done]/;p;}")" != "[Done]" ]; do
#echo "$data" | sed -n -e "/<\!--/{s/^.*<\!--.*-->.*\$/[Done]/;p;}"
#data="$(echo "$data" | sed -e "/<\!--/{N;s/\n//;}")"
#done
data="$(echo -n "$data" | sed -e "s/<\!--/\/*/")"
data="$(echo -n "$data" | sed -e "s/-->/*\//")"
...
data="$(echo -n "$data" | sed -e "/\/\/ --- begin List/,/\/\/ --- end List/{s/(/(size_t i, /;s/, )/)/;s/{/{;;\n;;;\n/;}")"
data="$(echo -n "$data" | sed -e "/\/\/ --- begin List/,/\/\/ --- end List/{/\/\/ --- begin List/{p;s/^.*\/\/ --- begin List//;s/ Getter.*\$//;y/ABCDEFGHIJKLMNOPQRSTUVWXYZ/abcdefghijklmnopqrstuvwxyz/;h;d;};    /;;;/{x;p;x;d;};  }")"
data="$(echo -n "$data" | sed -e "/\/\/ --- begin List/,/\/\/ --- end List/{/;;/{N;s/;;\n/ /;N;s/\n /[i]./;};}")"
#data="$(echo -n "$data" | sed -e "/\[i\]\.return/{s/[^ {]\.return /return &/;s/\.return /./;}")"
data="$(echo -n "$data" | sed -e "/\[i\]\.return/{s/[^ {]*\.return /return &/;s/\.return /./;}")"
...
data="$(echo -n "$data" | sed -e "s/\t/ /g;s/<\!--.*-->//g;s/<\/message-implementation>/#endif \/\/ _${nameuc}MESSAGES_H_\n/;s/ <\/message>/};\n/g;s/^ *private:/private:/;")"
echo -n "$data
" > ${name}messages.h
...
data="$(echo -n "$data" | sed -e "/::serialise/,/}/{/[ \t]{/,/};/{s/^.*\$/<list>&/;};}")"
data="$(echo -n "$data" | sed -e "/::deserialise/,/}/{/[ \t]{/,/};/{s/^.*\$/<delist>&/;};}")"
data="$(echo -n "$data" | sed -e "s/<delist><list>/<list>/;")"
