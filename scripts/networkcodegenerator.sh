if [ "$1" = "" ]; then
  ...
  echo ""
  echo "this script is still incomplete (lacking some datatypes and the <list> feature, and should generate some more files) but if you want to automate the whole process you can specify -auto as filename to generate all messages automatically (assumes you are aither in PT/scripts/ or PT/)"
  exit
fi
if [ "$1" = "-auto" ]; then
...
while [ "$(echo "$data" | grep "<UC>")" != "" ]; do
  UC="$(echo "$data" | grep "<UC>" -m 1 | sed -e "s/^.*<UC>//;s/<\/UC>.*\$//;y/abcdefghijklmnopqrstuvwxyz/ABCDEFGHIJKLMNOPQRSTUVWXYZ/;")"
...
while [ "$(echo "$data" | grep "<LC>")" != "" ]; do
  LC="$(echo "$data" | grep "<LC>" -m 1 | sed -e "s/^.*<LC>//;s/<\/LC>.*\$//;y/ABCDEFGHIJKLMNOPQRSTUVWXYZ/abcdefghijklmnopqrstuvwxyz/;")"
...
# Tripple each colour24 tag since they repeat trice to get R, G and B
data="$(echo -n "$data" | sed -e "/<colour24>/{p;p;}")"

data="$(echo -n "$data" | sed -e "/<message /,/<\/message>/{s/^.* name=\"/void /;s/\">/Message::serialise(ByteStream* bs)\n{\n  Serialiser serial(bs);\n  serial.setInt8(type);\n  serial.setInt8(id);/;s/<uint32>/serial.setInt32(<LC>/;s/<string>/serial.setString(<LC>/;s/<text>/serial.setString(<LC>/;s/<bool>/serial.setInt8(<LC>/;s/<colour24>/serial.setInt8(<LC>/;s/<vector3f>/serial.setFloat(<LC>/;s/<\/uint32>/<\/LC>);/;s/<\/string>/<\/LC>);/;s/<\/text>/<\/LC>);/;s/<\/bool>/<\/LC>?1:0);/;s/<\/colour24>/<\/LC>[#fixme#]);/;s/<\/vector3f>/<\/LC>[#fixme#]);/;}")"
data="$(echo -n "$data" | sed -e "/<demessage /,/<\/demessage>/{s/^.* name=\"/void /;s/\">/Message::deserialise(ByteStream* bs)\n{\n  Deserialiser serial(bs);\n  type = serial.getInt8();\n  id = serial.getInt8();/;s/<uint32>/<LC>/;s/<string>/<LC>/;s/<text>/serial.getString(<LC>/;s/<bool>/<LC>/;s/<colour24>/<LC>/;s/<vector3f>/<LC>/;s/<\/uint32>/<\/LC> = (unsigned int) serial.getInt32();/;s/<\/string>/<\/LC> = serial.getString();/;s/<\/text>/<\/LC>);/;s/<\/bool>/<\/LC> = serial.getInt8() != 0;/;s/<\/colour24>/<\/LC>[#fixme#] = (unsigned char) serial.getInt8();/;s/<\/vector3f>/<\/LC>[#fixme#] = serial.getFloat();/;}")"
