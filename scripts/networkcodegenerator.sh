...
data="$(cat "$1" | sed -e "s/\r//")"
# Some compatibility stuff
data="$(echo -n "$data" | sed -e "s/><\/message>/>\n<\/message>/")"
data="$(echo -n "$data" | sed -e "s/<\!--/\/*/")"
data="$(echo -n "$data" | sed -e "s/-->/*\//")"
data="$(echo -n "$data" | sed -e "/^\$/d")"
messages="$(echo "$data" | grep "<message " | sed -e "s/^.* name=\"/    /;s/\">/=[ENUM],/;y/abcdefghijklmnopqrstuvwxyz/ABCDEFGHIJKLMNOPQRSTUVWXYZ/;" | head -c -2)"
i=0 # Hack to set the enum-numbers right
while [ "$(echo -n "$messages" | grep "\[ENUM\]")" != "" ]; do
  line="$(echo -n "$messages" | sed -n -e "/\[ENUM\]/=" | head -n 1 | head -c -1)"
  messages="$(echo -n "$messages" | sed -e "$line s/\[ENUM\]/$i/")"
  i=$(($i+1))
done
...
# Some list stuff
data="$(echo -n "$data" | sed -e "/<list name=\"/{p;s/^[^<]*<list name=\"//;s/\">//;h;d;};/<\/list>/{x;s/^/  <endlist name=\"/;s/\$/\">/;}")"

data="$(echo -n "$data" | sed -e "/^class /{p;s/^class //;s/ : .*/;;/;h;d;};/<[^/]*>/{x;p;x;}")" # This is to move the variables to the top of the function
data="$(echo -n "$data" | sed -e "/;;/{N;s/\n//;}")"

# Add the list objects to the constructor and descructor functions of the class it belongs to
eval "$(echo -n "$data" | sed -n -e "/;;[^<]*<list/{s/^.*;;/&_listsC=\"\${&_listsC}\\\n    <LC>/;s/;;//g;s/[\t ]*<list name=\"//g;s/\">/<\\\\\/LC> = 0;\"/;p;}")"
eval "$(echo -n "$data" | sed -n -e "/;;[^<]*<list/{s/^.*;;/&_listsD=\"\${&_listsD}\\\n    delete [] <LC>/;s/;;//g;s/[\t ]*<list name=\"//g;s/\">/<\\\\\/LC>;\"/;p;}")"
eval "$(echo "$data" | sed -n -e "/^class /{s/^class //;s/ .*//;s/^.*\$/data=\"\$(echo -n \"\$data\" | sed -e \"s\/\\\\$&_listsC\/$&_listsC\/\")\"/;p;}")"
eval "$(echo "$data" | sed -n -e "/^class /{s/^class //;s/ .*//;s/^.*\$/data=\"\$(echo -n \"\$data\" | sed -e \"s\/\\\\$&_listsD\/$&_listsD\/\")\"/;p;}")"

data="$(echo -n "$data" | sed -e "s/<uint32>.*<\/uint32>/private:unsigned int <LC>&<\/LC>;\n  unsigned int get&() { return <LC>&<\/LC>; }\n  void set&(unsigned int x) { <LC>&<\/LC> = x; }\n/")"
data="$(echo -n "$data" | sed -e "s/<uint16>.*<\/uint16>/private:unsigned short <LC>&<\/LC>;\n  unsigned short get&() { return <LC>&<\/LC>; }\n  void set&(unsigned short x) { <LC>&<\/LC> = x; }\n/")"
data="$(echo -n "$data" | sed -e "s/<uint8>.*<\/uint8>/private:unsigned char <LC>&<\/LC>;\n  unsigned char get&() { return <LC>&<\/LC>; }\n  void set&(unsigned char x) { <LC>&<\/LC> = x; }\n/")"
data="$(echo -n "$data" | sed -e "s/<float>.*<\/float>/private:float <LC>&<\/LC>;\n  float get&() { return <LC>&<\/LC>; }\n  void set&(float x) { <LC>&<\/LC> = x; }\n/")"
data="$(echo -n "$data" | sed -e "s/<string>.*<\/string>/private:ptString <LC>&<\/LC>;\n  ptString get&() { return <LC>&<\/LC>; }\n  void set&(ptString x) { <LC>&<\/LC> = x; }\n/")"
data="$(echo -n "$data" | sed -e "s/<text>.*<\/text>/private:const char* <LC>&<\/LC>;\n  const char* get&() { return <LC>&<\/LC>; }\n  void set&(const char* x) { <LC>&<\/LC> = x; }\n/")"
data="$(echo -n "$data" | sed -e "s/<bool>.*<\/bool>/private:bool <LC>&<\/LC>;\n  bool get&() { return <LC>&<\/LC>; }\n  void set&(bool x) { <LC>&<\/LC> = x; }\n/")"
data="$(echo -n "$data" | sed -e "s/<colour24>.*<\/colour24>/private:unsigned char <LC>&<\/LC>[3];\n  unsigned char* get&() { return <LC>&<\/LC>; }\n  void set&(unsigned char r, unsigned char g, unsigned char b)\n  {\n    <LC>&<\/LC>[0] = r;\n    <LC>&<\/LC>[1] = g;\n    <LC>&<\/LC>[2] = b;\n  }\n  void set&(const unsigned char* x)\n  {\n    set&(x[0], x[1], x[2]);\n  }\n/")"
data="$(echo -n "$data" | sed -e "s/<vector3f>.*<\/vector3f>/private:float <LC>&<\/LC>[3];\n  float* get&() { return <LC>&<\/LC>; }\n  void set&(float x, float y, float z)\n  {\n    <LC>&<\/LC>[0] = x;\n    <LC>&<\/LC>[1] = y;\n    <LC>&<\/LC>[2] = z;\n  }\n  void set&(const float* x)\n  {\n    set&(x[0], x[1], x[2]);\n  }\n/")"

data="$(echo -n "$data" | sed -e "s/<list [^>]*>/private:List:&\n  unsigned char get&Count() { return <LC>&<\/LC>count; }\n  void set&Count(unsigned char ic)\n  {\n    <LC>&<\/LC>count = ic;\n    delete [] <LC>&<\/LC>;\n    <LC>&<\/LC> = new List&[ic];\n  }\n\n  \/\/ --- begin List& Getter and Setter ---\n/")"
data="$(echo -n "$data" | sed -e "/<endlist name=\"/{s/<endlist name=\"[^>]*>/private:EndList:&\n  \/\/ --- end List& Getter and Setter ---\n/g;}")"
...
data="$(echo -n "$data" | sed -e "/<list name=\"/{s/<list name=\"//g;s/\">//g;}")"
data="$(echo -n "$data" | sed -e "/<endlist name=\"/{s/<endlist name=\"//g;s/\">//g;}")"
# Resolve all <LC> tags again, but add an "#L#" in front of it.. for lists
while [ "$(echo "$data" | grep "<LC>" -c)" != "0" ]; do
  ...
  data="$(echo -n "$data" | sed -e "$line s/<LC>.*<\/LC>/#L#$LC/")"
done
# Move the variable declarations
#eval "$(echo -n "$data" | sed -n -e "/;;/{s/;;[^p]*private:/_variables/;s/^.*_variables/&=\"$\{&\}/;s/\$/\"/;p;}")"
eval "$(echo -n "$data" | sed -n -e "/;;/{s/;;[^p]*private:/_variables/;s/^.*_variables/&=\"$\{&\}\\\n  /;s/\$/\"/;p;}")"
#eval "$(echo -n "$data" | sed -n -e "/^class /{s/^class /echo \"¤/;s/ : .*//;s/¤.*\$/&=\$&_variables\"/;s/¤//g;p;}")"
eval "$(echo -n "$data" | sed -n -e "/^class /{s/^class /data=\"\$(echo -n \"\$data\" | sed -e \"s\/\\\\\$\\{¤/;s/ : .*//;s/¤.*\$/&_variables\}\/\$&_variables\/\")\"/;s/¤//g;p;}")"
# Remove the old variable declarations
data="$(echo -n "$data" | sed -e "/;;/d")"
# List-stuff
data="$(echo -n "$data" | sed -e "s/  List:.*\$/  class List&\n  {\n  public:/;s/  List://g;")"
data="$(echo -n "$data" | sed -e "s/  EndList:.*\$/  };\n\n  unsigned char <LC>&<\/LC>count;\n  List&* <LC>&<\/LC>;/;s/  EndList://g;")"
data="$(echo -n "$data" | sed -e "/^[\t ][\t ]class List/,/};/{/public:/,/};/{s/^/  /;};s/  public:/public:/;s/  };/};/;}")"
data="$(echo -n "$data" | sed -e "/\/\/ --- begin List/,/\/\/ --- end List/{s/(/(size_t i, /;s/, )/)/;s/#L#/;;\n;;;\n#L#/;}")"
data="$(echo -n "$data" | sed -e "/\/\/ --- begin List/,/\/\/ --- end List/{/{/,/}/{/^[^{}]*\$/s/size_t i, /i, /;};/\/\/ --- begin List/{p;s/^.*\/\/ --- begin List//;s/ Getter.*\$//;y/ABCDEFGHIJKLMNOPQRSTUVWXYZ/abcdefghijklmnopqrstuvwxyz/;h;d;};    /;;;/{x;p;x;d;};  }")"
data="$(echo -n "$data" | sed -e "/\/\/ --- begin List/,/\/\/ --- end List/{/;;/{N;s/;;\n//;N;s/\n/[i]./;};}")"
data="$(echo -n "$data" | sed -e "s/#L#//g")"

# Resolve all new <LC> tags
while [ "$(echo "$data" | grep "<LC>" -c)" != "0" ]; do
  LC="$(echo "$data" | grep "<LC>" | head -n 1 | sed -e "s/^.*<LC>//;s/<\/LC>.*\$//;y/ABCDEFGHIJKLMNOPQRSTUVWXYZ/abcdefghijklmnopqrstuvwxyz/;")"
  line="$(echo -n "$data" | sed -n -e "/<LC>/=" | head -n 1 | head -c -1)"
  data="$(echo -n "$data" | sed -e "$line s/<LC>.*<\/LC>/$LC/")"
done
data="$(echo -n "$data" | sed -e "s/\t/ /g;s/<\!--.*-->//g;s/<\/message-implementation>/#endif \/\/ ${nameuc}MESSAGES_H\n/;s/ <\/message>/};\n/g;s/<\/message>/};\n/g;s/^ *private:/private:/;")"
echo -n "$data
" > ${name}messages.h
...
data="$(echo -n "$data" | sed -e "s/<\/list>/};\n/")"

data="$(echo -n "$data" | sed -e "/<message /,/<\/message>/{/<list name=/{s/<list name=/<list=/;s/\">//;};s/^.* name=\"/void /;s/\">/Message::serialise(ByteStream* bs)\n{\n  Serialiser serial(bs);\n  serial.setInt8(type);\n  serial.setInt8(id);/;s/<uint32>/serial.setInt32(<LC>/;s/<uint16>/serial.setInt16(<LC>/;s/<uint8>/serial.setInt8(<LC>/;s/<float>/serial.setFloat(<LC>/;s/<string>/serial.setString(<LC>/;s/<text>/serial.setString(<LC>/;s/<bool>/serial.setInt8(<LC>/;s/<colour24>/serial.setInt8(<LC>/;s/<vector3f>/serial.setFloat(<LC>/;s/<\/uint32>/<\/LC>);/;s/<\/uint16>/<\/LC>);/;s/<\/uint8>/<\/LC>);/;s/<\/float>/<\/LC>);/;s/<\/string>/<\/LC>);/;s/<\/text>/<\/LC>);/;s/<\/bool>/<\/LC>?1:0);/;s/<\/colour24>/<\/LC>[#fixme#]);/;s/<\/vector3f>/<\/LC>[#fixme#]);/;}")"
data="$(echo -n "$data" | sed -e "/<demessage /,/<\/demessage>/{/<list name=/{s/<list name=/<delist=/;s/\">//;};s/^.* name=\"/void /;s/\">/Message::deserialise(ByteStream* bs)\n{\n  Deserialiser serial(bs);\n  type = serial.getInt8();\n  id = serial.getInt8();/;s/<uint32>/<LC>/;s/<uint16>/<LC>/;s/<uint8>/<LC>/;s/<float>/<LC>/;s/<string>/<LC>/;s/<text>/serial.getString(<LC>/;s/<bool>/<LC>/;s/<colour24>/<LC>/;s/<vector3f>/<LC>/;s/<\/uint32>/<\/LC> = (unsigned int) serial.getInt32();/;s/<\/uint16>/<\/LC> = (unsigned short) serial.getInt16();/;s/<\/uint8>/<\/LC> = (unsigned char) serial.getInt8();/;s/<\/float>/<\/LC> = serial.getFloat();/;s/<\/string>/<\/LC> = serial.getString();/;s/<\/text>/<\/LC>);/;s/<\/bool>/<\/LC> = serial.getInt8() != 0;/;s/<\/colour24>/<\/LC>[#fixme#] = (unsigned char) serial.getInt8();/;s/<\/vector3f>/<\/LC>[#fixme#] = serial.getFloat();/;}")"

data="$(echo -n "$data" | sed -e "s/<list=\".*\$/serial.setInt8(<LC>&<\/LC>count);\n  for ( size_t i = 0; i < <LC>&<\/LC>count ; i++ )\n  { \/\/ Listname: <LC>&<\/LC>/;s/<list=\"//g;")"
data="$(echo -n "$data" | sed -e "s/<delist=\".*\$/<LC>&<\/LC>count = (unsigned char) serial.getInt8();\n  set&Count(<LC>&<\/LC>count);\n  for ( size_t i = 0; i < <LC>&<\/LC>count ; i++ )\n  { \/\/ Listname: <LC>&<\/LC>/;s/<delist=\"//g;")"

data="$(echo -n "$data" | sed -e "/[ \t]{/,/};/{s/^.*\$/<list>&/;}")"
data="$(echo -n "$data" | sed -e "/::deserialise/,/<\/demessage>/{s/<list>/<delist>/;}")"
data="$(echo -n "$data" | sed -e "/<list>[ \t]*{/{s/<list>//;};")"
data="$(echo -n "$data" | sed -e "/<list>[ \t]*}/{s/<list>//;};")"
data="$(echo -n "$data" | sed -e "/<delist>[ \t]*{/{s/<delist>//;};")"
data="$(echo -n "$data" | sed -e "/<delist>[ \t]*}/{s/<delist>//;};")"
#data="$(echo -n "$data" | sed -e "/<list>/{s/<list>[ \t]*/    /;s/(/({nobr}\n{listname}\n[i]./;}")"
data="$(echo -n "$data" | sed -e "/<list>/{s/<list>[ \t]*/    /g;s/<LC>/{nobr}\n{listname}\n[i].<LC>/;}")"
#data="$(echo -n "$data" | sed -e "s/<delist>[ \t]*/    {nobr}\n{listname}\n[i]./;")"
data="$(echo -n "$data" | sed -e "/<delist>/{s/<delist>[ \t]*/    /g;s/<LC>/{nobr}\n{listname}\n[i].<LC>/;}")"
...
data="$(echo -n "$data" | sed -e "/\/\/ Listname: /{s/^.* \/\/ Listname: //;s/\$/{nobr}/;h;s/^.*\$/  {/;};/{listname}/{x;p;x;d;}")"
data="$(echo -n "$data" | sed -e "/{nobr}/{N;s/{nobr}\n//;}")"
data="$(echo -n "$data" | sed -e "/{nobr}/{N;s/{nobr}\n//;}")"
...
data="$(echo -n "$data" | sed -e "s/\t/ /g;s/<\!--.*-->//g;s/ <\/message>/}/g;s/ <\/demessage>/}/g;s/<\/message>/}/g;s/<\/demessage>/}/g;s/ *\$//;")"
while [ "$(echo "$data" | grep "<LC>" -c)" != "0" ]; do
  LC="$(echo "$data" | grep "<LC>" | head -n 1 | sed -e "s/^.*<LC>//;s/<\/LC>.*\$//;y/ABCDEFGHIJKLMNOPQRSTUVWXYZ/abcdefghijklmnopqrstuvwxyz/;")"
  line="$(echo -n "$data" | sed -n -e "/<LC>/=" | head -n 1 | head -c -1)"
  data="$(echo -n "$data" | sed -e "$line s/<LC>.*<\/LC>/$LC/")"
