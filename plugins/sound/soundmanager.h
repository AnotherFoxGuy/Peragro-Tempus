...
class SoundManager
  : public scfImplementation3<SoundManager, iSoundManager, iEventHandler,
      iComponent>
{
private:
...
/*
Source material is found by a check for material done first on the source
object's ammo, then on the contents (if any) of the weapon hand, and finally
on the actor template itself. With this in mind, an archer could have a source
of "arrow", a spellcaster could have a source of "fireball", a sword-wielding
fighter could have a source of "steelsword", and an unarmed krug scout could
have a source of "flesh" (fists).

Destination material is found by a check for material done first on the
destination object's shield, followed by a check for material on the torso
armor, and finally on the actor's template itself. For example, if a warrior
wearing plate armor is holding a wooden buckler, the warrior's destination
material is "wood" (the material specification of the shield). However, if
that same warrior then dropped the shield, the destination material becomes
"steel" (the material specification of the plate armor).
*/
