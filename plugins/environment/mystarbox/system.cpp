...
{
  // Convert the star pos into cs_unit position to match camera space.
  csVector3 cs_unit_pos = system_pos * SB_LY_CSUNIT;
  // Get the camera pos.
  csVector3 cam_pos = c->GetTransform().GetT2OTranslation();
...
void System::Calculate_Position()
{
  ...
  system_pos.z = distance * cos(right_ascension * PI / 180) *
    cos(declination * PI / 180);
  system_pos.x = distance * sin(right_ascension * PI / 180) *
    cos(declination * PI / 180);
  // Where dis = distance to star from Sun.
  system_pos.y = distance * sin(declination * PI / 180);
...
void System::DrawNebulaTexture(const iCamera* c, iGraphics3D* g3d)
{
  ...
  csVector3 starpos(system_pos.x * SB_SCALE_LY_UNIT,
    system_pos.y * SB_SCALE_LY_UNIT, system_pos.z * SB_SCALE_LY_UNIT);
  ...
  colors[0].Set(1, 1, 1, 0.7);
  colors[1].Set(1, 1, 1, 0.7);
  colors[2].Set(1, 1, 1, 0.7);
  colors[3].Set(1, 1, 1, 0.7);
