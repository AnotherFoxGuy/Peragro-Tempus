...
private:
  ...
  // Declaring this below tex_nebula causes a seg fault in Add_Star at
...
public:
  ...
  // Distance from camera in light years.
  float Get_Distance ( const iCamera* c );
  ...
  bool Add_Star(std::string new_name,
    std::string classification,
    float new_luminosity,
    int color,
    iTextureWrapper* tex);

  bool Add_Nebula(std::string tex_nebula_name,
    float w,
    float h,
    iEngine* eng);
  ...
  int GetPixelX(const int size)
  {
    return static_cast<int>((right_ascension / 360) * size);
  }
  int GetPixelY(const int size)
  {
    return static_cast<int>((((declination / 180) * size) + (size / 2)));
  }

};
#endif // _H_STARBOX_SYSTEM_
