...
bool TerrainEd::Initialize (iObjectRegistry* r)
{
  ...
  CEGUI::Window* win = winMgr->createWindow("ice/FrameWindow", "Frame");
  ...
  CEGUI::Window* box = winMgr->createWindow("ice/Combobox", "Frame/Combobox");
  ...
  CEGUI::Window* list = winMgr->createWindow("ice/StaticImage", "Frame/Listbox");
...
CEGUI::Window* TerrainEd::CreateShaderSpinner(uint i, const char* name, csArray<float>& vals, bool flt)
{
  ...
  CEGUI::Window* win = winMgr->createWindow("ice/StaticImage", wname.GetData());
  ...
  CEGUI::Window* text = winMgr->createWindow("ice/StaticText", tname.GetData());
  ...
  for (int t = 0; t < (int)vals.GetSize(); t++)
  {
    ...
    CEGUI::Window* spinner = winMgr->createWindow("ice/Spinner", rname.GetData());
