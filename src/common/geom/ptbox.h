...
/**
 * \name Indices of corner vertices for PtBox.
 * Used by PtBox::GetCorner().
 * @{ */
enum
{
  /// min X, min Y, min Z
...
public:
  ...
  inline const PtVector3& Min () const { return minbox; }
  /// Get the 3d vector of maximum (x, y, z) values
  inline const PtVector3& Max () const { return maxbox; }
   ...
   */
  PtVector3 GetCorner (int corner) const;

  /// Get the center of this box.
  inline PtVector3 GetCenter () const { return (minbox+maxbox)/2; }

  /**
   * Set the center of this box. This will not change the size
   * of the box but just relocate the center.
   */
  void SetCenter (const PtVector3& c);

  /// Set the size of the box but keep the center intact.
  void SetSize (const PtVector3& s);

  /// Get the size of the box 
  inline PtVector3 GetSize () const { return (maxbox-minbox); }
  ...
  inline void StartBoundingBox (const PtVector3& v)
  {
    minbox = v; maxbox = v;
  }

  /// Add a new vertex and recalculate the bounding box.
  inline void AddBoundingVertex (const PtVector3& v)
  {
  ...
  PtBox (const PtVector3& v) : minbox (v), maxbox (v) 
  { }

  /// Initialize this box with two points.
  PtBox (const PtVector3& v1, const PtVector3& v2) :
  	minbox (v1), maxbox (v2)
  { if (Empty ()) StartBoundingBox (); }

  /// Sets the bounds of the box with the given values.
  inline void Set (const PtVector3& bmin, const PtVector3& bmax)
  {
    minbox = bmin;
    maxbox = bmax;
  }
  ...
  bool Contains (const PtVector3& v) const;
