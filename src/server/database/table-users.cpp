...
void UsersTable::insert(const char* name, const char* pwhash)
{
  if (strlen(name) + strlen(pwhash) > 512) assert("Strings too long");
  char query[1024];
  sprintf(query, "insert into users (name, pwhash) values ('%s','%s');", name, pwhash);
  db->update(query);
}
...
bool UsersTable::existsUser(const char* name)
{
...
User* UsersTable::getUser(const char* name)
{
  if (strlen(name)> 512) assert("String too long");
  char query[1024];
  ...
  user->setName(rs->GetData(0,1).c_str(), rs->GetData(0,1).length());
