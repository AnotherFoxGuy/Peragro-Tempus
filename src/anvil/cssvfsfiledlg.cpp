...
#include "images/folder_closed.xpm"
#include "images/folder_open.xpm"
#include "images/new_archive.xpm"
...
SCF_IMPLEMENT_IBASE(cssVFSFileDlg)
...
//Create widgets
	...
	newArchiveButton = new wxBitmapButton( this,
	  BUTTON_NEW_ARCHIVE,
	  wxBitmap (new_archive_xpm),
	  wxDefaultPosition,
	  wxSize(25,25) );
	newArchiveButton->SetToolTip( _("Create Archive") );
...
//Set Objects into sizers
	...
	boxSizerTopRight->Add( newArchiveButton, 0, wxALIGN_CENTER|wxRIGHT, 5 );
...
bool cssVFSFileDlg :: HasChildDir( wxTreeItemId parent )
{
	...
	if( childList->Length() > 0 && !strcmp( childList->Get(0), path))
		return false;

	size_t numChildren = childList->Length();
	size_t childIndex;

	for( childIndex = 0; childIndex < numChildren; childIndex++ )
	{
		const char* childPath = childList->Get( childIndex );
		if( IsDir( const_cast< char* > (childPath) ) )
		{
			return true;
			break;
		}/*IF*/
...
bool cssVFSFileDlg :: IsDir( char* path )
{
	char lastChar = 0;
...
bool cssVFSFileDlg :: IsFile(char* path )
{
	char lastChar = 0;
	lastChar = path[ strlen( path ) -1 ];
...
bool cssVFSFileDlg :: LoadVFSDirView(char *path)
{
  if(!engine)
    return false;
  ...
  long nentries = (long) entry->Length();
  int id= -1;

  for(long i = 0; i < nentries; i++)
  {
  //  char *curentry = (char*)entry->Get(i);
		wxString name =  entry->Get(i);	
...
bool cssVFSFileDlg :: LoadVFSTreeBranch(wxTreeItemId parent, char *path)
{
  if(!engine)
    return false;
  ...
  size_t numberEntries = childList->Length();
  size_t entryIndex;

  for( entryIndex = 0; entryIndex < numberEntries; entryIndex++ )
  {
		wxString curTestPath = childList->Get( entryIndex );
		if( curTestPath.Last() == '/' )
    {
...
void cssVFSFileDlg :: OpenTreeItemChild( wxString childName )
{
  ...
  {		
    LoadVFSTreeBranch(curTreeItem, (char*)curDirPath.GetData());
  }
...
void cssVFSFileDlg :: OnDirTreeExpandBranch( wxTreeEvent& event )
{
	...
	{		
		LoadVFSTreeBranch(curSelection, (char*)curDirPath.GetData());
	}

	//Select the item that was expanded
	dirtree->SelectItem( curSelection );
	thepath->SetValue(curDirPath.GetData());
}

void cssVFSFileDlg :: OnDirTreeSelChange(wxTreeEvent& event)
{
  csString path = GetPathByTreeItem(event.GetItem());
  LoadVFSDirView((char*)path.GetData());
	curdvpath = path;
}
...
{
  filename = fileView->GetItemText( event.GetIndex() );

  wxString fullpath = curdvpath.GetData();
  fullpath.Append(filename);

  thepath->SetValue(fullpath.GetData());
}
void cssVFSFileDlg :: OnOk( wxCommandEvent& event )
{
	...
	if( IsFile( (char*)filename.GetData() ) )
	{
		//Returns to main frame to be opened
		EndModal(TRUE);
	}
}
