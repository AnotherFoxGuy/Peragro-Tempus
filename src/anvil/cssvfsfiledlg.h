...
*/
#ifndef __CSSVFSFILEDLG_H_INCLUDED__
#define __CSSVFSFILEDLG_H_INCLUDED__
...
class cssVFSFileDlg : public wxDialog, public iBase
{
public:
  ...
  ~cssVFSFileDlg();
	...
	const char*  GetPathData() { return curdvpath.GetData(); }
	const char*  GetFileNameData() { return filename.GetData(); }
...
//Enumerators
  ...
  csString GetPathByTreeItem(wxTreeItemId id);
	...
	bool LoadVFSDirView(char *path);
	bool LoadVFSTreeBranch(wxTreeItemId parent, char *path);

	void OpenTreeItemChild( wxString childName );
//Crystal Space instance
	anvEngine* engine;
...
//Frame Elements
  ...
  wxChoice *filter;
	...
	wxListCtrl *fileView;
	wxChoice *viewTypes;
	wxBitmapButton *newArchiveButton;
