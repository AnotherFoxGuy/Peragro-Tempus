...
namespace PT
{
  ...
  bool DialogConfiguration::LoadConfiguration (const std::string& file)
  {
    ...
    while (it->HasNext())
    {
      ...
      if (sscanf(parts.at(0).c_str(), "%f", &xScale) == 0 || sscanf(parts.at(1).c_str(), "%f", &xOffset) == 0)
      {
        continue;
      }
      if (sscanf(parts.at(2).c_str(), "%f", &yScale) == 0 || sscanf(parts.at(3).c_str(), "%f", &yOffset) == 0)
      {
        continue;
      }

      dialogConfig.userPosition = CEGUI::UVector2(CEGUI::UDim(xScale, xOffset), CEGUI::UDim(yScale, yOffset));
      ...
      dialogConfig.userSize = CEGUI::UVector2(CEGUI::UDim(xScale, xOffset), CEGUI::UDim(yScale, yOffset));
  ...
  bool DialogConfiguration::SaveConfiguration (const std::string& fileName)
  {
    ...
    for (; ppkNode != ppkEnd; ++ppkNode)
    {
      ...
      csRef<iDocumentNode> dialogNode = dialogs->CreateNodeBefore(CS_NODE_ELEMENT);
      dialogNode->SetValue("dialog");
      dialogNode->SetAttribute("name", wnd->getName().c_str());

      csRef<iDocumentNode> position = dialogNode->CreateNodeBefore(CS_NODE_ELEMENT);
      position->SetValue("position");

      csRef<iDocumentNode> positionText = position->CreateNodeBefore(CS_NODE_TEXT);

      std::stringstream str;
      CEGUI::UVector2 pos = wnd->getPosition();
      str << pos.d_x.d_scale << "," << pos.d_x.d_offset << "," << pos.d_y.d_scale << "," << pos.d_y.d_offset;
      positionText->SetValue(str.str().c_str());

      csRef<iDocumentNode> size = dialogNode->CreateNodeBefore(CS_NODE_ELEMENT);
      size->SetValue("size");

      csRef<iDocumentNode> sizeText = size->CreateNodeBefore(CS_NODE_TEXT);

      CEGUI::UVector2 sz = wnd->getSize();
      str.str("");
      str << sz.d_x.d_scale << "," << sz.d_x.d_offset << "," << sz.d_y.d_scale << "," << sz.d_y.d_offset;
      sizeText->SetValue(str.str().c_str());

      csRef<iDocumentNode> visible = dialogNode->CreateNodeBefore(CS_NODE_ELEMENT);
      visible->SetValue("visible");

      csRef<iDocumentNode> visibleText = visible->CreateNodeBefore(CS_NODE_TEXT);
      if (wnd->isVisible())
      {
  ...
  bool DialogConfiguration::SetDialogToDefault (CEGUI::Window * window, bool position, bool size, bool visible)
  {
    WindowMap::iterator result = dialogConfigurations.find(window);
  ...
  bool DialogConfiguration::SetDialogToDefault (const std::string& windowName, bool position, bool size, bool visible)
  {
    try
    {
  ...
  std::vector<std::string> DialogConfiguration::split (std::string str, std::string delimiter)
  {
