...
namespace CEGUI
{
  ...
  CEGUI_DEFINE_WINDOW_FACTORY(SettingComboBox)
  SettingComboBox::SettingComboBox(const String& type, const String& name) : SettingBase(type, name)
  {
  ...
  void SettingComboBox::Update()
  {
    ...
    else
    {
      ...
      if (box->getItemCount() == 0)
      {
        ...
        {
          CEGUI::ListboxItem* item = new CEGUI::ListboxTextItem(keys[i], (CEGUI::uint)i);
  ...
  void SettingComboBox::initialiseComponents(void)
  {
    ...
    box->subscribeEvent(Combobox::EventListSelectionAccepted, Event::Subscriber(&CEGUI::SettingComboBox::onSelectionAccepted, this));
