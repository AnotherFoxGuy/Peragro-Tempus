...
public:
	...
	void set_modelview_matrices(const csMatrix3& _mat_earth_equ_to_eye,
	...
	virtual bool project_custom(const csVector3& v, csVector3& win, const csMatrix3& mat) const
	{
		//gluProject(v[0],v[1],v[2],mat,mat_projection,vec_viewport,&win[0],&win[1],&win[2]);
		return (win[2]<1.);
	}

    bool project_custom_check(const csVector3& v, csVector3& win, const csMatrix3& mat) const
    ...
    bool project_custom_line_check(const csVector3& v1, csVector3& win1,
					       const csVector3& v2, csVector3& win2, const csMatrix3& mat) const
...
protected:
	...
	csMatrix3 mat_projection;		// Projection matrix

	csVector3 center;				// Viewport center in screen pixel
	double view_scaling_factor;	// ??
	double flip_horz,flip_vert;

	csMatrix3 mat_earth_equ_to_eye;		// Modelview Matrix for earth equatorial projection
	csMatrix3 mat_j2000_to_eye;         // for precessed equ coords
	csMatrix3 mat_helio_to_eye;			// Modelview Matrix for earth equatorial projection
	csMatrix3 mat_local_to_eye;			// Modelview Matrix for earth equatorial projection
	csMatrix3 inv_mat_earth_equ_to_eye;	// Inverse of mat_projection*mat_earth_equ_to_eye
	csMatrix3 inv_mat_j2000_to_eye;		// Inverse of mat_projection*mat_earth_equ_to_eye
	csMatrix3 inv_mat_helio_to_eye;		// Inverse of mat_projection*mat_helio_to_eye
	csMatrix3 inv_mat_local_to_eye;		// Inverse of mat_projection*mat_local_to_eye

	// transformation from screen 2D point x,y to object
	// m is here the already inverted full tranfo matrix
	virtual
    void unproject(double x, double y, const csMatrix3& m, csVector3& v) const
	{
